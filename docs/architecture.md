...
- 外部システム連携（Git、ファイル、環境変数、コマンド実行）は `infrastructure` で扱う。
- UI は表示とユーザー操作に集中し、依存解決や外部実装の詳細を持たない。
...
- Git 実行、ファイル I/O、環境変数読込、キャッシュ永続化などを実装する。
- `domain` 判断ロジックは持たない。
- 例: `worktree-store.ts`, `worktree-pr-infra.ts`, `repository-mapping-store.ts`,
  `deleted-worktree-store.ts`。

### UI

- ユースケース呼び出し、状態管理、表示制御を行う。
- 依存は `composition-root.ts` から受け取る。
