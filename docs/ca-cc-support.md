...
- **cc** = Claude Code。これから対応する。現時点では未対応。
...
- `composition-root.ts` の注入はすべて `codex-session-file-store` 由来:
  - `loadWorktreeDeckTitlesSnapshotDependencies` ← `loadTitlesForPaths`, `attachWorktreeTitles`
  - `worktreeSessionFileDependencies` ← `findFirst/LatestSessionFileByPath`, `loadSessionMessages` 等
  - **ここが cc provider を差し込む口**。

## 5. cc（Claude Code）の差分（未対応）
...
- subagent は同一ファイル内の行の `isSidechain` フィールドで判別する想定（Task/サブエージェント使用時に `true` の行が混在。本環境のログでは未観測）。
...
- 待ち: `AskUserQuestion` / `ExitPlanMode` / permission 未承認の tool_use を、`tool_use.id` ↔ user content の `tool_result.tool_use_id` で突き合わせて未完了を追跡する（ca の `waitingForUserCallIds` と同型）。
- mtime 経過（`WORKTREE_DECK_DONE_THRESHOLD_DAYS`）で working→done に倒すのは ca と共通。
- review の概念は無いため、sessionKind は main / subagent のみに簡素化（ca の review 重複除外ロジックは不要）。
...
- 新規（ファイル名は案）: `domain/claude-session-log-parser.service.ts` — cc 行パーサ。`ParsedSessionLog` を出力。
- 新規（案）: `infrastructure/claude-session-file-store.ts` — projects 収集 + キャッシュ + 紐付け。
- 変更: 共通型を provider 非依存へ整理（`worktree-types.ts` の re-export と `application/worktree-title.entity.ts` の `WorktreeTitle` 二重定義の両方を解消）。
- 変更: `loadTitlesForPaths` 相当を ca/cc マージに、`WorktreeTitle` に provider 種別を追加。
- 変更: `composition-root.ts` で両 provider を束ねる。
- UI: 一覧 / 詳細 / メニューバーで provider を区別表示。
...
- cc の探索ルート設定: 既定 `~/.claude/projects` を Preferences で上書き可能にするか、env 名をどうするか。
...
- `WorktreeTitle` の provider 種別の型（`"ca" | "cc"`）と UI 表現（アイコン / ラベル）。
