...
- Claude の手動 open-app 連携（worktree を開く際の `claude-app` 起動など）。Auto Start は §10 で対応済み。
- Codex の起動・App 連携（`codex-app-*`, open-app 系）。
- cc 詳細メッセージ loader。
- CI 設定（本ブランチでは未変更）。

## 10. Auto Start（cc の自動起動）

Codex 同様に Claude でもバックグラウンド自動起動に対応する。Auto Start は worktree 作成フォーム
（`components/worktree-create-form.tsx`）から provider を選び、detached worker
（`assets/auto_start_worker.js`）が裏で実行する。
...
- `cwd=worktreePath` により session JSONL が `~/.claude/projects/<cwd変換>/<id>.jsonl` に作られ、
  §5 の cc 収集がそのまま拾う。
- stream-json の `system/init` から `session_id` を取り出した時点で job state を `starting-turn` にし
  （`threadId` フィールドに session_id を保存）、turn 完了まで待って成否を判定する。
- 認証失敗等は exit code だけでなく stdout の result イベント / system error / stderr から英語メッセージを抽出する。
- Claude Agent SDK は採用しない。SDK は内部で `claude` CLI を spawn するラッパーで認証問題を解決せず、
  raw asset では npm 依存も解決できないため。Codex 同様「CLI を shell-out」に揃える。

### 10.3 認証（最重要）

- macOS の Claude 認証は Keychain 保存。GUI(Raycast)から `detached`/`stdio:ignore` で起動された worker は
  Keychain を非対話で参照できず `Not logged in` になる。Codex は `~/.codex/auth.json`（ファイル）なので worker から読めて成立していた。
- 対策: `claude setup-token` で長期 OAuth token を発行し、Raycast Preference `CLAUDE_CODE_OAUTH_TOKEN`（password）に保存。
  `applyRaycastPreferencesToProcessEnv()` で `process.env` に載り、worker → `claude` 子プロセスへ env 継承で透過する。
  subscription 課金を維持できる。秘匿値なので job state JSON には書かない。
- 代替の `ANTHROPIC_API_KEY` は API クレジット課金になり、優先順位も OAuth token より上なので併設しない。

### 10.4 branch/title 生成

- cc でも branch 名・セッションタイトルの生成は `codex exec`（read-only sandbox, 既存実装）を流用する。
  生成は命名だけで安定・安価なため。将来 Claude 生成へ寄せるのは別タスク。

### 10.5 画像添付

- cc では `claude -p` の headless 実行に画像を渡さない。Codex の `localImage` 相当は未対応。
- 誤添付を防ぐため、provider が cc のときはフォームの画像添付 UI（Images サマリ項目と Attach/Preview/Clear アクション）を非表示にする（`buildCreateWorktreeFormItemOrder` で `imagePaths` を ca 限定、ActionPanel は `showImageAttachments = autoStart && provider !== "cc"` で出し分け）。

### 10.6 デプロイ経路（検証時の注意）

- `npm run dev`（`RAY_Target=x ray develop`）は Raycast X（`~/.config/raycast-x/...`）へ asset を配置する。
- `ray build`（`npm run build` / `check:all`）は classic Raycast（`~/.config/raycast/...`）へ配置する。
- worker を手コピーすると検証対象と修正対象がズレるので、実機検証は worker を含めて `npm run dev` 側へ確実に反映してから行う。

### 10.7 セッションの再開（resume）

- **重要な仕様**: `claude -p`（headless）で作ったセッションは `claude --resume` の対話 picker や `claude -c`（--continue）に**出ない**（picker は対話セッションのみ列挙）。global picker でも当該 worktree でも出ない。
- ただし **session id を明示すれば再開できる**: `claude --resume <session_id>`（現在の project とその worktree を検索）。session id は cc セッション JSONL のファイル名（`~/.claude/projects/<cwd変換>/<session_id>.jsonl`）。
- worktree-deck 側の対応: 一覧項目に **Copy Claude Resume Command (First Session)（⌘⇧R）** を追加。`claude --resume <id>` を組み立ててクリップボードへコピーする（`application/worktree-title.entity.ts` の `resolveClaudeResumeCommand`）。worktree を開いた端末に貼って再開する想定。端末/エディタ非依存にするためワンクリック起動ではなくコピー方式を採る。
- **対象は「最初（最古）の cc セッション」**。用途が「Auto Start で仕込んだセッションを後で再開する」ことであり、Auto Start は worktree を新規作成して最初のセッションを作るため、最初のセッション = Auto Start セッションになる。判定は `startedAt` 最小（無ければ `updatedAt`）。後から resume すると `updatedAt` は新しくなるので、最新ではなく開始時刻で選ぶ。複数セッションがあっても最初のものが対象であることをアクション名（First Session）で明示する。
